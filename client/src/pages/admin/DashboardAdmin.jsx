...
export default function DashboardAdmin() {
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main flex flex-col">
      ...
      {isSistemTerkunci && (
        <div className="w-full bg-danger/10 border-b border-danger/20 text-danger text-tiny font-black text-center py-2.5 uppercase tracking-wider select-none animate-pulse">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border-default/50 pb-4">
          <div className="space-y-1">
            ...
            <p className="text-tiny text-text-muted font-medium uppercase tracking-widest">
          ...
          <div className="flex items-center gap-2.5 whitespace-nowrap shrink-0 h-9 justify-end sm:ml-auto">
            <div className="h-full flex items-stretch">
              <Badge
                ...
                className="h-full rounded-[--radius-md] px-4 flex items-center justify-center"
              >
            ...
            <div className="h-full flex items-stretch">
              <Button
                ...
                disabled={isSistemTerkunci}
                className={`h-full py-0 px-4 text-caption font-black shadow-[--shadow-card] flex items-center justify-center gap-1.5 border-0 rounded-[--radius-md] text-white select-none disabled:opacity-30 disabled:cursor-not-allowed ${
                  !isAdminTutup
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <section className="lg:col-span-2 space-y-6">
            ...
            >
              ...
              <p className="text-caption font-bold text-text-muted uppercase tracking-[0.2em] mb-4">
                Nomor Sedang Dilayani
              </p>
              <div className="w-full max-w-sm bg-bg-muted-box border border-border-default h-28 flex items-center justify-center rounded-3xl shadow-[--shadow-inner]">
                <span
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mt-8">
                <Button
                  ...
                  className="flex flex-col items-center justify-center gap-2 h-24 rounded-[--radius-lg] border-2 py-0 font-bold disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <Volume2
                    size={24}
                ...
                <Button
                  ...
                  className="flex flex-col items-center justify-center gap-2 h-24 rounded-[--radius-lg] shadow-[--shadow-modal] shadow-brand-primary/20 scale-105 py-0 font-black disabled:opacity-30 disabled:cursor-not-allowed"
                >
            ...
            >
              <div className="bg-emerald-500/5 border border-emerald-500/10 p-5 rounded-[--radius-lg] flex items-center gap-4">
                <div className="w-10 h-10 rounded-[--radius-md] bg-emerald-500 text-white flex items-center justify-center">
                  <Users size={20} />
                </div>
                <div>
                  <p className="text-tiny font-bold text-emerald-600 uppercase tracking-widest">
                    Sisa Antrean
                  </p>
              ...
              <div className="bg-amber-500/5 border border-amber-500/10 p-5 rounded-[--radius-lg] flex items-center gap-4">
                <div className="w-10 h-10 rounded-[--radius-md] bg-amber-500 text-white flex items-center justify-center">
                  <Clock size={20} />
                </div>
                <div>
                  <p className="text-tiny font-bold text-amber-600 uppercase tracking-widest">
                    Estimasi Selesai
                  </p>
                  <p className="text-xl font-black text-text-main leading-none mt-1">
          ...
          <section className="space-y-6">
            ...
            >
              <div className="p-4 border-b border-border-default bg-bg-muted-box">
                <h3 className="text-caption font-bold text-text-muted uppercase tracking-widest">
                  Antrean Berikutnya
                </h3>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-thin">
                {!isAdminTutup && isAdminLoggedIn ? (
                  <>
                    {nomorAktif !== "—" && (
                      <div className="flex items-center justify-between p-3 rounded-[--radius-md] border border-brand-primary/30 bg-brand-primary/5 w-full">
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="text-tiny font-black px-1.5 py-0.5 rounded-[--radius-sm] bg-brand-primary text-white">
                            1
                          </span>
                          <span className="text-tiny font-bold text-brand-primary">
                            Aktif Dilayani
                          </span>
                        </div>
                    ...
                    {(() => {
                      ...
                      return displayList.length > 0 ? (
                        displayList.map((nomor, index) => (
                          <div
                            key={index}
                            className="flex items-center justify-between p-3 rounded-[--radius-md] border border-border-default bg-bg-main/50 opacity-75 w-full"
                          >
                            <div className="flex items-center gap-2 shrink-0">
                              <span className="text-tiny font-bold px-1.5 py-0.5 rounded-[--radius-sm] bg-border-default text-text-muted">
            ...
            >
              <div className="p-4 border-b border-border-default bg-amber-500/5">
                <h3 className="text-caption font-bold text-amber-600 uppercase tracking-widest">
                  Daftar Lewati (Hold)
                </h3>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-thin">
                ...
                isAdminLoggedIn ? (
                  daftarDilewati.map((ticket, index) => (
                    <div
                      key={ticket.id || index}
                      className="flex items-center justify-between p-3 rounded-[--radius-md] border border-amber-500/20 bg-amber-500/5"
                    >
      ...
      >
        <div className="space-y-4 py-1 text-center">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Button
              variant={!isAdminTutup ? "destructive" : "default"}
              onClick={handleConfirmStatusToggle}
            >
              Ya, {!isAdminTutup ? "Tutup" : "Buka"}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-4 py-1 text-center">
          ...
          <div className="space-y-1">
            <p className="text-tiny font-bold text-text-main">
      ...
      >
        <div className="space-y-3 py-1">
          <p className="text-caption text-text-muted text-center leading-normal">
              ...
              .map((meja) => {
                ...
                return (
                  <button
                    ...
                    disabled={isDipakai}
                    className={`w-full flex items-center justify-between p-3.5 text-left rounded-[--radius-md] border ${
                      isDipakai
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-[--radius-sm] bg-brand-primary/10 border border-brand-primary/20 flex items-center justify-center text-brand-primary text-tiny font-black">
                        {meja.kode_loket}
                      </div>
                      ...
                      {isMejaTujuanTutup && (
                        <span className="text-micro font-black px-1.5 py-0.5 rounded-[--radius-sm] bg-rose-500/10 text-rose-500 border border-rose-500/20 uppercase tracking-wider scale-90">
                          Tutup
                        </span>
                      )}
                    </div>
                    <span className="text-tiny font-bold text-brand-primary opacity-0 group-hover:opacity-100 transition-all">
                      Pilih →
                    </span>
                  </button>
                );
