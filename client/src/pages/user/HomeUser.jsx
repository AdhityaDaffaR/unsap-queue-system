...
export default function HomeUser() {
  ...
  const isGilirankuDipanggil =
    punyaTiketAktif && layananAktif.aktifDisplay === nomorTiketBaru;
  ...
  const getButtonProps = () => {
    if (isMonitorUtamaTutup) {
      return {
        ...
        disabled: true,
        className:
          "opacity-40 bg-gray-400 dark:bg-zinc-700 cursor-not-allowed pointer-events-none",
      };
    }
    if (punyaTiketAktif) {
      return {
        ...
        disabled: true,
        className:
          "opacity-50 bg-emerald-600 text-white cursor-not-allowed pointer-events-none border-0",
      };
    }
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main flex flex-col">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 space-y-6 mt-4">
        ...
        {punyaTiketAktif && (
          <section className="w-full transition-all duration-300">
            ...
            >
              <div
                className={`absolute top-0 left-0 h-full w-1 ${loketPemanggil ? "bg-success" : "bg-brand-primary"}`}
              />
              ...
              <div className="flex items-center gap-3 w-full md:w-auto shrink-0 justify-end">
                ...
                >
                  ...
                  <p
                    className={`text-2xl font-black mt-1 leading-none tracking-tight ${loketPemanggil ? "text-success" : "text-brand-primary"}`}
                  >
                    {nomorTiketBaru}
                  </p>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4 mt-4 w-full h-9">
                <h1 className="text-lg sm:text-xl font-black tracking-tight text-text-main flex items-center gap-1.5 truncate">
                  ...
                  <span className="text-text-muted/80 font-bold">
                    ({layananAktif.subTeks})
                  </span>
                </h1>

                <Badge
                  variant={!isMonitorUtamaTutup ? "success" : "danger"}
                  className="shrink-0"
                >
                  <div className="flex items-center gap-1">
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${!isMonitorUtamaTutup ? "bg-success animate-pulse" : "bg-danger"}`}
                    />
                ...
                loketPemanggil.kategori === selectedKategori ? (
                  ...
                  <div className="flex flex-col justify-between h-full flex-1">
                    <div className="flex flex-col items-center justify-center flex-1 py-1">
                      ...
                      <div
                        className={`text-5xl sm:text-6xl font-black tracking-tighter leading-none mt-2 ${
                          isMonitorUtamaTutup
                            ? "text-text-muted/40"
                            : "text-brand-primary"
                        }`}
                          ...
                          <div className="w-full bg-emerald-600 dark:bg-emerald-500 text-white py-2 px-4 flex items-center justify-center gap-1.5 text-sm font-black uppercase tracking-wide shadow-[--shadow-inner] select-none animate-[pulse_2s_infinite]">
                            <span>
                              ...
                              {konterFisikPemanggil
                                ? konterFisikPemanggil.kode
                                : layananAktif.kodeDisplay
                                    .replace("LOKET ", "")
                                    .split(" / ")[0]}
                            </span>
                          </div>
          ...
          <section className="md:col-span-1">
            <Card
              className={`h-[380px] flex flex-col p-6 overflow-hidden relative transition-all duration-200 ${isMonitorUtamaTutup ? "opacity-65" : ""}`}
            >
                ...
                {!isMonitorUtamaTutup && layananAktif.selanjutnyaList.length > 0
                  ? layananAktif.selanjutnyaList.map((nomor, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1 text-right text-base font-black tracking-tight text-text-muted/90">
                          {nomor}
                        </div>
                      </div>
                    ))
        ...
        <section className="w-full space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-xs font-bold tracking-tight uppercase text-text-muted">
              Pilih Kategori Transaksi Layanan Kampus
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {layananList.map((layanan) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-0 left-0 w-full h-1 ${isTutup ? "bg-danger" : "bg-success"}`}
                  />

                  <div className="space-y-3 flex-1 flex flex-col min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3
                        className={`text-xs font-black tracking-tight uppercase truncate ${isSelected ? "text-brand-primary" : "text-text-main"}`}
                      >
                    ...
                    <div className="w-full overflow-hidden whitespace-nowrap relative h-[18px] flex items-center bg-transparent">
                      <p
                        className={`text-xs font-semibold tracking-tight text-text-muted absolute ${
                          layanan.subTeks.length > 15
                            ? "animate-[cardMarquee_12s_linear_infinite] pl-[10%] hover:[animation-play-state:paused]"
                            : ""
                        }`}
                      >
                        {layanan.subTeks}
                  ...
                  <div className="mt-4 pt-2 border-t border-border-default flex items-center justify-between text-micro w-full">
                    <span className="text-text-muted font-medium">
                      {isTutup
                        ? "Tutup"
                        : `Sisa antrean: ${layanan.sisa} orang`}
                    </span>
                    <span
                      className={`transition-all flex items-center font-bold gap-0.5 ${isSelected ? "text-brand-primary opacity-100" : "text-text-muted opacity-0 group-hover:opacity-100 group-hover:text-brand-primary"}`}
                    >
      ...
      <Modal
        ...
        title="Akses Pengambilan Tiket Ditolak"
      >
      ...
      <Modal
        ...
        title="Nomor Antrean Berhasil Diambil"
      >
        <div className="text-center flex flex-col items-center space-y-4 py-2">
